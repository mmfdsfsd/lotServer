...
function Install()
{
  ...
  URLKernel='https://github.com/mengyalei/lotServer/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/mengyalei/lotServer/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/mengyalei/lotServer/raw/master/lotServer.tar"
  ...
  #wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "https://118868.xyz/keygen.php?ver=${acce_ver}&mac=${Mac}"
   wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "https://lotserverkey.free-ssr.gq/keygen.php?ver=${acce_ver}&mac=${Mac}"
